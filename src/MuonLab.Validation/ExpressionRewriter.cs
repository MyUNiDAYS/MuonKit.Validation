...
namespace MuonLab.Validation
{
	...
	sealed class ExpressionRewriter
	{
		...
		readonly Dictionary<Expression, Expression> subst;
		...
		static IEnumerable<Expression> CoalesceTerms(
		...
		Expression Walk(Expression expression)
		{
			if (expression == null) return null;
			Expression tmp;
			if (this.subst.TryGetValue(expression, out tmp)) return tmp;
